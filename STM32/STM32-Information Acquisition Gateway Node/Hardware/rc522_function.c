...
void IC_test(void)
{
	...
	//**************************************************************//
	//在这里存放卡号
	u8 ID1[]={0x00,0x03,0x8c,0x67};		//卡号uid：00 03 8c 67
	u8 ID2[]={0x00,0x05,0x99,0x73};		//卡号uid：00 05 99 73
	//**************************************************************//
  while (1)
  {    
		/*寻卡*/
      ...
      /*防冲撞（当有多张卡进入读写器操作范围时，防冲突机制会从其中选择一张进行操作）*/
			if ( PcdAnticoll ( ucArray_ID ) == MI_OK )                                                                   
			{
				...
				if((ID1[0]==ucArray_ID[0]||ID2[0]==ucArray_ID[0])&&(ID1[1]==ucArray_ID[1]||ID2[1]==ucArray_ID[1])&&(ID1[2]==ucArray_ID[2]||ID2[2]==ucArray_ID[2])&&(ID1[3]==ucArray_ID[3]||ID2[3]==ucArray_ID[3]))
				{
					TIM_SetCompare2(TIM2, 1500);
					OLED_ShowString(3,1,"Successful!     ");
					OLED_ShowString(4,1,"Open the Door!  ");
					u1_printf("Menu7: The Door is OPEN \r\n");
					Delay_ms(3000);
				...
				else 
				{
					...
					u1_printf("Menu7: The Door is CLOSE \r\n");
					GPIO_SetBits(LED1_GPIO_PORT, LED1_GPIO_PIN);
					Delay_ms(1000);
					GPIO_ResetBits(LED1_GPIO_PORT,LED1_GPIO_PIN);
					Delay_ms(1000);
