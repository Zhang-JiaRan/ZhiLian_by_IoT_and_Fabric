...
//char timer1_flag = 0;   //1:定时器4中断，有数据到
...
//{
//	TIM_TimeBaseInitTypeDef TIM_TimeBaseInitStructure2;              //定义一个设置定时器的变量
//	NVIC_InitTypeDef NVIC_InitStructure2;                            //定义一个设置中断的变量
//	
//	NVIC_PriorityGroupConfig(NVIC_PriorityGroup_2);                 //设置中断向量分组：第2组 抢先优先级：0 1 2 3 子优先级：0 1 2 3		
//	RCC_APB1PeriphClockCmd(RCC_APB1Periph_TIM4, ENABLE);            //使能TIM4时钟	
//    TIM_TimeBaseInitStructure2.TIM_Period = arr; 	                //设置自动重装载值
//	TIM_TimeBaseInitStructure2.TIM_Prescaler = psc;                  //设置定时器预分频数
//	TIM_TimeBaseInitStructure2.TIM_CounterMode = TIM_CounterMode_Up; //向上计数模式
//	TIM_TimeBaseInitStructure2.TIM_ClockDivision = TIM_CKD_DIV1;     //1分频
//	TIM_TimeBaseInit(TIM1, &TIM_TimeBaseInitStructure2);             //设置TIM4
//	
//	TIM_ClearITPendingBit(TIM1, TIM_IT_Update);                     //清除溢出中断标志位
//	TIM_ITConfig(TIM1, TIM_IT_Update, ENABLE);                      //使能TIM4溢出中断    
//	TIM_Cmd(TIM1, DISABLE);                                         //先关闭TIM4                          
//	
//	NVIC_InitStructure2.NVIC_IRQChannel = TIM4_IRQn;                 //设置TIM4中断
//	NVIC_InitStructure2.NVIC_IRQChannelPreemptionPriority = 1;       //抢占优先级1
//	NVIC_InitStructure2.NVIC_IRQChannelSubPriority = 0;              //子优先级0
//	NVIC_InitStructure2.NVIC_IRQChannelCmd = ENABLE;                 //中断通道使能
//	NVIC_Init(&NVIC_InitStructure2);                                 //设置中断
//}
